...
powershell_script 'Configure Local XREF Groups' do
  code <<-EOH
    ...
    $isoUserName = "#{node.run_state['secret1']['BTS_SvcHostIsoAccount']}"
...
powershell_script 'Configure FinSuite Host Instances' do
  ...
  c:\\setup\\bts\\Configure-FinSuiteHostsInstances.ps1 -User $userName -Domain $userDomain -Password $userPassword
  [System.IO.File]::WriteAllText("c:\\setup\\bts\\_chefFinHostInstancesConfigured.lock", "complete")
...
log "#{cookbook_name}::FSP SOA FTP Handlers Configured"
